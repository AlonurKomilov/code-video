...
export async function check({name, unit='', measure, pass, calibrate, note}){
 ...
 if(calibrate){
  ...
  catch(e){ cal = null; sep = false; }
 }
 R.push({name,unit,value,ok,cal,sep,note,
...
export function report(){
 ...
 for(const r of R){
  ...
  if(r.status==='UNPROVEN') console.log('      the broken case passed too — this check cannot see the failure it watches');
